...
int main() {
    ...
    const auto reconnect_start =
        std::chrono::steady_clock::now();

    if (!consumer.reconnect(6, 250)) {
        std::cerr << "Consumer reconnect failed\n";
    ...
    const auto reconnect_end =
        std::chrono::steady_clock::now();

    const auto reconnect_us =
    ...
    const auto failure_window_us =
        std::chrono::duration_cast<
            std::chrono::microseconds
        >(
            reconnect_start - disconnect_start
        ).count();
