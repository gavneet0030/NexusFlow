...
bool KafkaConsumerAdapter::reconnect(
...
std::uint64_t KafkaConsumerAdapter::reconnect_count() const {
    return reconnect_count_.load(std::memory_order_acquire);
}
...
bool KafkaConsumerAdapter::poll(
    ...
    if (message->err != RD_KAFKA_RESP_ERR_NO_ERROR) {
        const rd_kafka_resp_err_t error = message->err;
        rd_kafka_message_destroy(message);

        if (error != RD_KAFKA_RESP_ERR__TIMED_OUT) {
            connected_.store(false, std::memory_order_release);
        }
